...
export const getItemDetail = async (entry: string, itemData: object) => {
...
export const getItemGroupDetail = async (group: string, itemData: object) => {
  ...
  for (const itemEntry of Object.keys(itemData[group])) {
    ...
    const detail = await getItemDetail(`${group}\t${itemEntry}`, itemData);
